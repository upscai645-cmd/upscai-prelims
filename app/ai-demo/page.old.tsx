      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-semibold">
              UPSC PYQ – AI Analysis Demo
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              Question from Supabase → your attempt → AI explanation layer.
            </p>
          </div>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            onClick={handleNext}
            disabled={
              loadingQuestion || questionIndex >= filteredQuestions.length - 1
            }
            className="rounded-md bg-slate-900 border border-slate-700 px-3 py-1 text-xs md:text-sm disabled:opacity-40"
          >
            Next →
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 space-y-4">
          ...
          {!loadingQuestion && !question && !error && (
            <p className="text-sm text-rose-400">
              No question found for this filter.
            </p>
          )}
        </section>
...
function AnalysisTabs({ analysis }: AnalysisTabsProps) {
  const [activeTab, setActiveTab] = useState<"solution" | "strategy">(
    "solution"
  );
    ...
    <section className="space-y-4">
      <div className="flex gap-2 border-b border-slate-700 pb-2">
        <button
          ...
          className={`${tabBase} ${
            activeTab === "solution" ? tabActive : tabInactive
          }`}
        >
        ...
        <button
          ...
          className={`${tabBase} ${
            activeTab === "strategy" ? tabActive : tabInactive
          }`}
        >
          Exam Strategy
        </button>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-emerald-600 bg-emerald-950/40 px-4 py-3 text-sm text-emerald-100">
            ...
            <div className="mt-1 text-lg font-bold">
              Option {analysis.correct_answer}
            </div>
          </div>
        ...
        <ul className="mt-2 space-y-1 text-sm text-slate-200 list-disc list-inside">
          ...
          {s.difficulty.why.length === 0 && (
            <li className="text-slate-400 list-none">
              No difficulty rationale returned.
            </li>
          )}
        </ul>
      </div>
